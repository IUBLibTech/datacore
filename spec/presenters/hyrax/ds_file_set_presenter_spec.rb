...
RSpec.describe Hyrax::DsFileSetPresenter do
  ...
  # NOTE:  relative_url_root function exactly the same in collection_presenter
  ...
  describe "#provenance_log_entries?" do
    context "calls Deepblue::ProvenancePath.path_for_reference" do
      ...
      it "returns value of path_for_reference" do
  ...
  describe "#link_name" do
    ...
    context "when current_ability.admin?" do
      before {
        allow(subject.current_ability).to receive(:admin?).and_return true
        allow(subject.current_ability).to receive(:can?).with(:read, 3000).and_return false
      }
    ...
    context "when current_ability.can? read" do
      before {
        allow(subject.current_ability).to receive(:admin?).and_return false
        allow(subject.current_ability).to receive(:can?).with(:read, 3000).and_return true
      }
      it "calls first_title" do
        expect(subject).to receive(:first_title)
        expect(subject.link_name).to eq "Red Hot Spectacular"
    ...
    context "when not admin or read" do
      before {
        allow(subject.current_ability).to receive(:admin?).and_return false
        allow(subject.current_ability).to receive(:can?).with(:read, 3000).and_return false
      }
  ...
  describe "#file_name" do
    ...
    context "when tombstone does not have a value and file_size_too_large_to_download?" do
      before {
        expect(subject).to receive(:file_size_too_large_to_download?).and_return true
      }
      it "return link_name" do
    ...
    context "when tombstone does not have a value and not file_size_too_large_to_download?" do
      before {
        expect(subject).to receive(:file_size_too_large_to_download?).and_return false
      }
