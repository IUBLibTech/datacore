...
RSpec.describe Hyrax::HomepagePresenter do
  ...
  describe "#display_share_button?" do
      expected_results = [OpenStruct.new( user_unregistered: true, not_logged_in: true, can_create: true, expected_result: true ),
                          OpenStruct.new( user_unregistered: true, not_logged_in: true, can_create: false, expected_result: true ),
      ...
      context "display_share_button? is #{factor[:display]} and create_many_work_types? is #{factor[:create]}" do |it|
        ...
        it "returns #{factor[:expected_result]}" do
          ...
          if factor[:display]
            ...
            expect(subject).not_to receive(:create_many_work_types?)
          end
  ...
  describe "#user_unregistered?" do
    context "when current user is a new record" do
      before {
        allow(subject).to receive(:current_ability).and_return OpenStruct.new(current_user: OpenStruct.new(new_record?: true))
      }
      it "returns true" do
        expect(subject).to receive(:current_ability)

        expect(subject.send(:user_unregistered?)).to eq true
      end
    end

    context "when current user is not a new record and is a guest" do
      before {
        allow(subject).to receive(:current_ability).and_return OpenStruct.new(current_user: OpenStruct.new(new_record?: false, guest?: true))
      }
      it "returns true" do
        expect(subject).to receive(:current_ability).twice

        expect(subject.send(:user_unregistered?)).to eq true
      end
    end

    context "when current user is not a new record or a guest" do
      before {
        allow(subject).to receive(:current_ability).and_return OpenStruct.new(current_user: OpenStruct.new(new_record?: false, guest?: false))
      }
      it "returns false" do
        expect(subject).to receive(:current_ability).twice

        expect(subject.send(:user_unregistered?)).to eq false
      end
    end

    # after {
    #   expect(subject).to have_received(:current_ability)
    # }
  end
end
