...
RSpec.describe Hyrax::AdminSetOptionsPresenter do
  ...
  describe "#select_options_non_default_admin" do
    ...
    before {
      subject.instance_variable_set(:@service, MockService.new([mock_admin_set1, mock_admin_set2]))
      allow(subject).to receive(:data_attributes).with( mock_admin_set1 ).and_return "data attributes 1"
      allow(subject).to receive(:data_attributes).with( mock_admin_set2 ).and_return "data attributes 2"
    }
    it "returns set(s) that include the default admin set" do
      expect(subject).to receive(:data_attributes).with( mock_admin_set1 )
      expect(subject).to receive(:data_attributes).with( mock_admin_set2 )

      expect(subject.select_options_non_default_admin).to eq [["Default Admin Set", "admin_set/default", "data attributes 1"],
                                                              ["Personal Admin Set", "brand_new_set", "data attributes 2"]]
    end
  end
