...
RSpec.describe Hyrax::CollectionPresenter do
  ...
  describe "#featured?" do
    context "when the featured instance variable has a value" do
      before {
        subject.instance_variable_set(:@featured, "to be determined")
      }
      it "returns the value" do
        expect(FeaturedCollection).not_to receive(:where)

        expect(subject.featured?).to eq "to be determined"
      end
    end

    context "when the featured instance variable has NO value" do
      before {
        subject.instance_variable_set(:@solr_document, OpenStruct.new(id: 12))
        allow(FeaturedCollection).to receive(:where).with(collection_id: 12).and_return(OpenStruct.new(exists?: true))
      }

      it "" do
        expect(subject.featured?).to eq true
        expect(subject.instance_variable_get(:@featured)).to eq true
      end
    end
  end
  ...
  describe "#user_can_feature_collections?" do
    context "when current_ability.can? returns true" do
      ...
      it "returns true" do
        expect(subject.user_can_feature_collections?).to eq true
      end
    end
    ...
    context "when current_ability.can? returns false" do
      ...
      it "returns false" do
        expect(subject.user_can_feature_collections?).to eq false
      end
    end
  end

  describe "#collection_featurable?" do
    options = [{"can_feature" => true,  "public" => true,  "result" => true },
               {"can_feature" => false, "public" => true,  "result" => false},
               {"can_feature" => true,  "public" => false, "result" => false},
               {"can_feature" => false, "public" => false, "result" => false}]

    options.each do |option|
      context "when user_can_feature_collections? is #{option["can_feature"]} and solr_document.public? is #{option["public"]}" do
        before {
          allow(subject).to receive(:user_can_feature_collections?).and_return option["can_feature"]
          subject.instance_variable_set(:@solr_document, OpenStruct.new(public?: option["public"]))
        }
        it "returns #{option["result"]}" do
          expect(subject.collection_featurable?).to eq option["result"]
        end
      end
    end
  end
  ...
  describe "#display_feature_link?" do
    actions = [{"featurable" => true,  "another" => true,  "featured" => true,  "result" => false },
               {"featurable" => true,  "another" => true,  "featured" => false, "result" => true  },
               {"featurable" => false, "another" => true,  "featured" => false, "result" => false },
               {"featurable" => true,  "another" => false, "featured" => false, "result" => false },
               {"featurable" => true,  "another" => false, "featured" => true,  "result" => false },
               {"featurable" => false, "another" => true,  "featured" => true,  "result" => false }]

    actions.each do |action|
      context "when collection_featurable? is #{action["featurable"]}, can_create_another? is #{action["another"]}, and featured is #{action["featured"]}" do
        before {
          allow(subject).to receive(:collection_featurable?).and_return action["featurable"]
          allow(FeaturedCollection).to receive(:can_create_another?).and_return action["another"]
          allow(subject).to receive(:featured?).and_return action["featured"]
        }

        it "returns #{action["result"]}" do
          expect(subject.display_feature_link?).to eq action["result"]
        end
      end
    end
  end
  ...
  describe "#display_unfeature_link?" do
    thoughts = [{"featurable" => true,  "featured" => true,  "result" => true },
                {"featurable" => false, "featured" => true,  "result" => false},
                {"featurable" => true,  "featured" => false, "result" => false},
                {"featurable" => false, "featured" => false, "result" => false}]

    thoughts.each do |thought|
      context "when collection_featurable? is #{thought["featurable"]} and featured? is #{thought["featured"]}" do
        before {
          allow(subject).to receive(:collection_featurable?).and_return thought["featurable"]
          allow(subject).to receive(:featured?).and_return thought["featured"]
        }
        it "returns #{thought["result"]}" do
          expect(subject.display_unfeature_link?).to eq thought["result"]
        end
      end
    end
  end
