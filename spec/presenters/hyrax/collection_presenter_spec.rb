...
RSpec.describe Hyrax::CollectionPresenter do
  ...
  describe "#initialize" do
    it "sets instance variables" do
      Hyrax::CollectionPresenter.new "solr", "current", "request"
  ...
  # NOTE:  relative_url_root function exactly the same in ds_file_set_presenter
    ...
    context "when @available_parents not present" do
      before {
        ...
        allow(Hyrax::Collections::NestedCollectionQueryService).to receive(:available_parent_collections).with(child: "la collection", scope: "scope", limit_to_id: nil).
          and_return [OpenStruct.new(id: 1, title: ["pancake", "flapjack"]), OpenStruct.new(id: 2, title: ["gnome", "gremlin"])]
      }
      it "set @available_parents and return as json" do
        ...
        expect(subject.available_parent_collections scope: "scope").to eq "[{\"id\":1,\"title_first\":\"pancake\"},{\"id\":2,\"title_first\":\"gnome\"}]"
        subject.instance_variable_get(:@available_parents) == [{"id" => 1, "title_first" => "pancake"}, {"id" => 2, "title_first" => "gnome"}]
      end
    end
  end
