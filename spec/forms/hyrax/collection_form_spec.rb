...
describe Hyrax::Forms::CollectionForm do
  ...
  describe "#permission_template" do
    before {
      allow(Hyrax::PermissionTemplate).to receive(:find_or_create_by).with(source_id: anything).and_return "template model"
      allow(Hyrax::Forms::PermissionTemplateForm).to receive(:new).with("template model")
    }
    it "calls PermissionTemplate.find_or_create_by and PermissionTemplateForm.new" do
      expect(Hyrax::PermissionTemplate).to receive(:find_or_create_by).with(source_id: anything).and_return "template model"
