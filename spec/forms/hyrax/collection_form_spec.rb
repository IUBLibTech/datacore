...
class MemberPresenterMock
  ...
  def file_set_presenters()
    MockFileSetPresenter.new(name: @name, id: @id)
  end
end
...
describe Hyrax::Forms::CollectionForm do
  ...
  describe "#all_files_with_access" do
    before {
      allow(subject).to receive(:member_work_ids).and_return "member work ids"
      allow(subject).to receive(:member_presenters).with("member work ids").and_return [MemberPresenterMock.new(name: "one", id: 1)]
    }
    it "returns array of arrays" do
      expect(subject.send(:all_files_with_access)).to eq [["one", 1]]
    end
  end

  describe "#member_work_ids" do
    before {
      allow(subject).to receive(:collection_member_service).and_return OpenStruct.new(available_member_work_ids: OpenStruct.new(response: MockResponse.new))
    }
    it "returns array of document ids" do
      expect(subject).to receive(:collection_member_service)
      expect(subject.send(:member_work_ids)).to eq ["Doc1", "Doc2", "Doc3"]
    end
  end

  describe "#collection_member_service" do
    context "when @collection_member_service is set" do
      before {
        subject.instance_variable_set(:@collection_member_service, "collection member service")
      }
      it "returns @collection_member_service" do
        expect(subject.membership_service_class).not_to receive(:new)
        expect(subject.send(:collection_member_service)).to eq "collection member service"
      end
    end

    context "when @collection_member_service is not set" do
      before {
        allow(subject).to receive(:scope).and_return "the scope"
        allow(subject).to receive(:collection).and_return "the collection"
        allow(subject.membership_service_class).to receive(:new)
                                                     .with(scope: "the scope", collection: "the collection", params: anything)
                                                     .and_return "new membership service"
      }
      it "calls membership_service_class.new" do
        expect(subject.send(:collection_member_service)).to eq "new membership service"
      end
    end
  end
  ...
  describe "#member_presenters" do
    before {
      allow(Hyrax::PresenterFactory).to receive(:build_for).with(ids: ["a1", "b2", "c3"], presenter_class: Hyrax::WorkShowPresenter, presenter_args: [nil])
                                                    .and_return "member presenters"
    }
    it "builds and returns member presenters" do
      expect(subject.send(:member_presenters, ["a1", "b2", "c3"]))
    end
  end
end
