...
RSpec.describe Deepblue::MigrationLogReporter do
  ...
  describe "#initialize" do
    before {
      allow(Deepblue::MigrationEventFilter).to receive(:new).and_return ["migration event filter"]
      allow(Deepblue::AndLogFilter).to receive(:new).with( filters: ["migration event filter"], options: { } ).and_return "and log filter"
    }

    context "when filter parameter is nil" do
      it "calls super" do
        expect(Deepblue::MigrationEventFilter).to receive(:new)
        Deepblue::MigrationLogReporter.new(filter: nil, input: "input", options: {})
      end
    end

    context "when filter parameter has a value" do
      it "calls super and filter_and method" do
        skip "Add a test"
      end
    end

    skip "Add a test for the value of @expected_ids_pathname"
  end
