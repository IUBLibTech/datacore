...
RSpec.describe IngestJob do
  ...
  describe "#perform" do
    before {
      ...
      allow(Deepblue::LoggingHelper).to receive(:obj_to_json).with("wrapper", anything).and_return "candy"
      ...
      allow(Deepblue::LoggingHelper).to receive(:bold_debug).with [ "here", "called from", anything, "candy", "notification=false",
    ...
    it "calls LoggingHelper.bold_debug and ingest_file" do
      ...
      expect(Deepblue::LoggingHelper).to receive(:bold_debug).with [ "here", "called from", anything, "candy", "notification=false",
                                                                    "continue_job_chain=true", "continue_job_chain_later=true", "delete_input_file=true",
                                                                    "uploaded_file=wrapper file upload", "File Upload",
                                                                    "uploaded_file.id=uploaded file ID",
                                                                    "uploaded_file_ids=[]", "bypass_fedora=false", "" ]
