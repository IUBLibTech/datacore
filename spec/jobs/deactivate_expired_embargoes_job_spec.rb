...
RSpec.describe DeactivateExpiredEmbargoesJob do
  ...
  describe "#perform" do
    ...
    before {
      ...
      allow(Deepblue::LoggingHelper).to receive(:obj_class).with('class', anything).and_return "class.class=Job"
    ...
    context "when verbose is false" do
      ...
      it "calls LoggingHelper.bold_debug and DeactivateExpiredEmbargoesService.new" do
        ...
        expect(Deepblue::SchedulerHelper).to receive(:log).with( class_name: "DeactivateExpiredEmbargoesJob", event: "deactivate_expired_embargoes" )
        expect(Deepblue::LoggingHelper).to receive(:bold_debug).with [ "here", "options={\"opaque\"=>nil}", "optional", ""]

        expect(subject).to receive(:job_options_value).with({"opaque"=>nil}, key: 'verbose', default_value: false )
        expect(subject).to receive(:job_options_value).with({"opaque"=>nil}, key: 'email_owner', default_value: true )
        expect(subject).to receive(:job_options_value).with({"opaque"=>nil}, key: 'skip_file_sets', default_value: true )
        expect(subject).to receive(:job_options_value).with({"opaque"=>nil}, key: 'test_mode', default_value: false )
    ...
    context "when an exception is raised calling DeactivateExpiredEmbargoesService.new" do
      before {
        allow(subject).to receive(:job_options_value).with({"opaque"=>nil}, key: 'verbose', default_value: false ).and_return false
        allow(Deepblue::DeactivateExpiredEmbargoesService).to receive(:new).with( email_owner: "email owner", skip_file_sets: "skip file sets",
                                                                                  test_mode: "test mode", verbose: false ).and_raise(Exception)
