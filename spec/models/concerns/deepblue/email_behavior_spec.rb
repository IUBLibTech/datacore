...
RSpec.describe Deepblue::EmailBehavior do
  ...
  describe "#email_event_notification" do
    context "when email_key_values are not blank, send_it is false, and return_email_parameters is false" do   # minimal
      before {
        allow(subject).to receive(:event_attributes_cache_write).with(event: "event", id: "M-99", behavior: :EmailBehavior)
        allow(subject).to receive(:email_compose_body).with(message: "message", email_key_values: "email key values").and_return "email body"
      }
      it "caches event attributes and does not send email" do
        expect(subject).to receive(:event_attributes_cache_write).with(event: "event", id: "M-99", behavior: :EmailBehavior)
        expect(subject).to receive(:email_compose_body).with(message: "message", email_key_values: "email key values").and_return "email body"
        expect(Deepblue::EmailHelper).not_to receive(:send_email)
        expect(Deepblue::EmailHelper).not_to receive(:log)
        expect(subject.send(:email_event_notification, to: "to", to_note: "note", from: "from", subject: "subject", attributes: "attributes",
                            current_user: "current user", event: "event", event_note: "event note", message: "message",
                            ignore_blank_key_values: true, id: "M-99", return_email_parameters: false, send_it: false,
                            email_key_values: "email key values")).to be_blank
      end
    end

    context "when email_key_values are blank, send_it is true, and return_email_parameters is true" do   # maximal
      before {
        allow(subject).to receive(:email_attribute_values_for_snapshot).with(attributes: "attributes", current_user: "current user",
                                                                             event: "event", event_note: "event note", to_note: "note", ignore_blank_key_values: true).and_return "snapshot" => "email values"
        allow(subject).to receive(:event_attributes_cache_write).with(event: "event", id: "N-98", behavior: :EmailBehavior)
        allow(subject).to receive(:email_compose_body).with(message: "message", email_key_values: {"snapshot" => "email values"})
                                                      .and_return "email body"
        allow(Deepblue::EmailHelper).to receive(:send_email).with( to: "to", from: "from", subject: "subject", body: "email body" )
        allow(subject).to receive(:for_email_class).and_return OpenStruct.new(name: "email class name")
      }
      it "calls email_attribute_values_for_snapshot, caches attributes, sends email, returns parameters" do
        expect(subject).to receive(:email_attribute_values_for_snapshot).with(attributes: "attributes", current_user: "current user",
                                                                              event: "event", event_note: "event note", to_note: "note", ignore_blank_key_values: true)
        expect(subject).to receive(:event_attributes_cache_write).with(event: "event", id: "N-98", behavior: :EmailBehavior)
        expect(subject).to receive(:email_compose_body).with(message: "message", email_key_values: {"snapshot" => "email values"})
        expect(Deepblue::EmailHelper).to receive(:send_email)
        expect(subject.send(:email_event_notification, to: "to", to_note: "note", from: "from", subject: "subject", attributes: "attributes",
                            current_user: "current user", event: "event", event_note: "event note", message: "message",
                            ignore_blank_key_values: true, id: "N-98", return_email_parameters: true, send_it: true,
                            email_key_values: "")).to eq to: "to", to_note: "note", from: "from", subject: "subject", message: "message",
                                                         body: "email body", current_user: "current user", event: "event", event_note: "event note",
                                                         id: "N-98", email_key_values: {"snapshot" => "email values"}
      end

      skip "Add a test for email_event_notification EmailHelper.log"
    end
  end
