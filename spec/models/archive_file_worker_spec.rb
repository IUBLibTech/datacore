...
RSpec.describe ArchiveFileWorker do
  ...
  pending "#self.job_files"

  pending "#self.too_many_jobs?"
  ...
  describe "#self.block_new_jobs?" do
    context "when too_many_jobs? is true" do
      ...
      it "returns true" do
        expect(ArchiveFileWorker).to receive(:too_many_jobs?).and_return true
    ...
    context "when too_many_jobs? is false" do
      ...
      context "when too_much_space_used? is true" do
        ...
        it "returns true" do
          expect(ArchiveFileWorker).to receive(:too_much_space_used?).and_return true
          expect(ArchiveFileWorker).to receive(:too_many_jobs?).and_return false
          expect(ArchiveFileWorker.block_new_jobs?).to eq true
        end
      end
      ...
      context "when too_much_space_used? is false" do
        ...
        it "returns true" do
          expect(ArchiveFileWorker).to receive(:too_much_space_used?).and_return false
          expect(ArchiveFileWorker).to receive(:too_many_jobs?).and_return false
  ...
  describe "#download_file" do
    ...
    context "when not too much space used" do
      ...
      context "when file download successful" do
        ...
        it "updates job, emails requesters, and logs info" do
          expect(subject.logger).to receive(:info).with "Download initiated for Yaml Path"
          expect(subject).to receive(:update_job_yaml).with({status: :staged_after_request})
          expect(subject).to receive(:update_job_yaml).with({transfer_started: "06/22/2025"})
          expect(subject).to receive(:system).with "curl command"
  ...
  describe "#email_requesters" do
    ...
    context "when Settings.archive_api.send_email has a value" do
      before {
      ...
      context "when user email present" do
        ...
        context "when error occurs sending email" do
          before {
            allow(Deepblue::EmailHelper).to receive(:send_email).with(to: "user@example.com", from: "contact@example.com",
              subject: "DataCORE file available for download: name of the file", body: "the email body", log: true).and_raise(StandardError, "Home-grown error")
          }
    ...
    context "when Settings.archive_api.send_email has no value" do
      before {
  ...
  describe "#sanitized_hash" do
    it "merges hash input" do
      expect(subject.sanitized_hash :user => "lebaby", :user_email => "lebaby@example.com", :other => "alternate").to eq :user => nil, :user_email => nil, :other => "alternate"
    end
  end
  ...
  describe "#delete_file?" do
    ...
    context "when not latest user download or when transfer not completed" do
      before {
