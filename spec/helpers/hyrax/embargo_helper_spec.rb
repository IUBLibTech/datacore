...
class ConcernMockery
  ...
  def initialize(embargo, fileset)
    @embargo = embargo
    @fileset = fileset
    @visibility = "open"
  end
...
RSpec.describe Hyrax::EmbargoHelper, type: :helper do
  ...
  describe "#days_to_embargo_release_date" do
    ...
    context "when embargo_release_date is a String" do
      it "parses embargo_release_date to a DateTime and returns difference in days between now parameter and embargo_release_date parameter" do
    ...
    context "when embargo_release_date is NOT a String" do
      it "returns difference in days between now parameter and embargo_release_date parameter" do
  ...
  describe "#deactivate_embargo" do
    ...
    context "when curation_concern parameter is not a FileSet" do
      ...
      curation = ConcernMockery.new(embargo, false)
      before {
    ...
    context "when curation_concern parameter is a FileSet, current_user parameter is nil, and verbose is true" do
      concerning = ConcernMockery.new(nil, true)
      before {
        allow(Deepblue::LoggingHelper).to receive(:debug).with "deactivate_embargo for file_set: curation concern id: CCID"
        allow(subject).to receive(:visibility_on_embargo_deactivation).with(curation_concern: concerning).and_return "institutional"
      }
      ...
      skip "Test provenance_unembargo and save!"
    end
  end
      ...
      context "when test_mode parameter is true and verbose is true" do
        it "logs and debugs but does NOT send email to author or notification_email" do
          ...
          expect(Deepblue::EmailHelper).to receive(:log).with( class_name: "EmbargoHelperMock",
                                                               ...
                                                               from: "notify email",
                                                               subject: "subject",
                                                               body: "body 1 body 2" )  # to from email addresses are different
  ...
  describe "#warn_deactivate_embargo_email" do
    it "returns nil" do
      expect(subject.warn_deactivate_embargo_email curation_concern: nil, days: nil).to be_nil
    end
  end
