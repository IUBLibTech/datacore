...
RSpec.describe Hyrax::Actors::FileSetActor do
  ...
  describe "#create_content" do
    ...
    context "when file_set.save is negative and file_set.title is blank" do
    ...
    context "when file_set.save is positive and from_url is false and file_set.label is nil and file_set.title is not blank" do
    ...
    context "when file_set.save is positive and from_url is true" do
  ...
  describe "#create_metadata" do
    ...
    context "when argument given is not a block and assign_visibility returns true" do
      before {
        allow(Deepblue::LoggingHelper).to receive(:bold_debug).with [ "here", "called from", "file_set_params={\"params\"=>\"file set\"}" ]
        allow(subject).to receive(:ability).and_return 'ability'
        allow(subject).to receive(:assign_visibility?).with({"params" => "file set"}).and_return true
        allow(subject).to receive(:ability).and_return 'ability'
        allow(Hyrax::Actors::Environment).to receive(:new).with(fileset, "ability", {"params" => "file set"}).and_return "test environment"
        allow(Hyrax::CurationConcern.file_set_create_actor).to receive(:create).with("test environment")
      }
      it "calls CurationConcern.file_set_create_actor.create and returns nil" do
        expect(Deepblue::LoggingHelper).to receive(:bold_debug).with [ "here", "called from", "file_set_params={\"params\"=>\"file set\"}" ]
        expect(fileset).to receive(:depositor=).with("depositor id")
        expect(fileset).to receive(:date_uploaded=).with("it's that time again")
        expect(fileset).to receive(:date_modified=).with("it's that time again")
    ...
    context "when argument given is a block" do
      it "returns file_set object" do
        skip "Add a test"
      end
    end
  end
  ...
  pending "#revert_content"


  # TODO: consolidate some subjects, create a standard one?
  ...
  describe "#label_for" do
    ...
    context "when file responds to function original_name" do
      it do
        expect(subject.send(:label_for, MockUploadedFile.new)).to eq "original name"
      end
    end

    context "when import_url present" do
      file_set = OpenStruct.new(import_url: "http://example.com")
      subject { described_class.new(file_set, "newish user") }

      before {
        allow(Addressable::URI).to receive(:parse).with( "http://example.com" ).and_return OpenStruct.new(path: "addressable")
        allow(File).to receive(:basename).with("addressable").and_return "import url file basename"
      }
      it do
        expect(subject.send(:label_for, "file")).to eq "import url file basename"
      end
    end
  ...
  describe "#remove_work_id_from_label" do
    context "when file_set.parent.id is present" do
      it "assigns label with file_set.parent.id removed" do
        file_set_mock = MockFileSet.new("shimmer_shine", "opalescent", "under the sea")
        subject.send(:remove_work_id_from_label, file_set_mock)
        file_set_mock.label == "shine"
      end
    end
  ...
  describe "#unlink_from_work" do
    ...
    context "when file_set.id is equal to thumbnail_id" do
      ...
      it "sets thumbnail to nil, removes file_set.id from rendering_ids, and calls save! on work" do
        ...
        parent.thumbnail.blank? == true
        parent.representative == "representative"
        parent.rendering_ids == ["rendering id", "representative id"]
      end
    end
    ...
    context "when file_set.id is equal to representative_id" do
      ...
      it "sets representative to nil, removes file_set.id from rendering_ids, and calls save! on work" do
        ...
        parent.representative.blank? == true
        parent.rendering_ids == ["rendering id", "thumbnail id"]
      end
    end
    ...
    context "when file_set.id is in rendering_ids" do
      ...
      it "removes file_set.id from rendering_ids, and calls save! on work" do
        ...
        parent.thumbnail == "thumbnail"
        parent.representative == "representative"
        parent.rendering_ids == ["thumbnail id", "thumbnail id"]
      end
    end
  end
