...
class MockCollectionMeasure

  def instance_variable_set( label, collection_id )
  end
end
...
RSpec.describe Deepblue::NewContentService do
  ...
  describe "#initialize" do
    ...
    # before {
    #   allow(Deepblue::NewContentService).to receive(:initialize_with_msg).with(options: {}, path_to_yaml_file: "yaml path", cfg_hash: "cfg hash",
    #                                                                            base_path: "base path" )
    # }
    # it "calls initialize_with_msg" do
    #   expect(Deepblue::NewContentService).to receive(:initialize_with_msg).with(options: { }, path_to_yaml_file: "yaml path", cfg_hash: "cfg hash",
    #                                                                             base_path: "base path" )
    #   Deepblue::NewContentService.new(path_to_yaml_file: "yaml path", cfg_hash: "cfg hash", base_path: "base path", options: {})
    # end
  end
  ...
  describe "#run" do
    ...
    context "when the function runs successfully" do
      before {
        allow(subject).to receive(:build_repo_contents)
      }
    ...
    content_service_errors.each do |error|
      context "when a #{error['name']} occurs" do
        before {
          allow(subject).to receive(:build_repo_contents).and_raise(error["name"], error["message"])
        }
        it "calls logger.error with error message" do
          unless error["name"] == Exception
    ...
    after {
      ...
      expect(subject).to have_received(:build_repo_contents)
    }
  end
  ...
  describe "#add_file_set_to_work" do
    context "when file_set parent parameter is present and work parameter id field equals file_set parameter parent_id field" do
      it "returns nil" do
        expect(subject.send(:add_file_set_to_work, work: OpenStruct.new(id: "ABC123"), file_set: OpenStruct.new(parent: "guardian", parent_id: "ABC123"))).to be_nil
      end
    end
    ...
    context "when work parameter id field NOT equal to file_set parameter parent_id field" do
      context "when file_set parent parameter is present and work parameter thumbnail field is present" do
        it "calls functions on work parameter" do
          ...
          expect(file_set_work).not_to receive(:thumbnail=).with(file_set)
      ...
      context "when file_set parent parameter is blank and work parameter representative_id is present" do
        it "calls functions on work parameter" do
          ...
          expect(file_set_work).not_to receive(:representative=).with(file_set)
          expect(file_set_work).to receive(:save!)
  ...
  describe "add_file_sets_to_work" do
    context "when work_hash parameter includes file_set_ids" do
      ...
      it "calls add_file_sets_to_work_from_file_set_ids and returns results" do
        ...
        expect(subject).not_to receive(:add_file_sets_to_work_from_files).with(work_hash: work_hash, work: "work")
    ...
    context "when work_hash parameter does NOT include file_set_ids" do
      ...
      it "calls add_file_sets_to_work_from_files and returns results" do
        expect(subject).not_to receive(:add_file_sets_to_work_from_file_set_ids).with(work_hash: work_hash, work: "work")
  ...
  describe "#add_file_sets_to_work_from_file_set_ids" do  # 216
  ...
  describe "#add_file_sets_to_work_from_files" do  # 241
    ...
    context "when files value in work_hash parameter is present" do
      context "when continue_new_content_service returns true" do
        ...
        context "when work_hash includes file ids and filenames" do
          work = MockFileSetWork.new
          before {
        ...
        context "when work_hash does NOT include file ids and filenames" do
          skip "add a test"
        end
      end
  ...
  describe "#add_measurement" do
    ...
    it "appends measurement parameter to measurements and returns measurements" do
    ...
    context "when works_from_hash call returns valid results" do
      ...
      context "when continue_new_content_service returns true" do
        ...
        before {
          ...
          allow(subject).to receive(:build_or_find_work).with(work_hash: "work hash 36", parent: collection).and_return MockCollectionWork.new ["collection id"]
        ...
        it "adds works to collection and returns collection parameter" do
          ...
          #expect(collection_work_42).to receive(save!)
          #expect(collection_work_42.member_of_collections).to eq [collection]
    ...
    context "when value for in_collections in work_hash parameter is present" do
      ...
      context "when collection can be found by id" do
        ...
        it "" do
          ...
          #expect(work.member_of_collections).to eq ["collection 101"]
  ...
  describe "#apply_visibility_and_workflow" do  # 333
    before {
  ...
  describe "#apply_work" do
    context "when work parameter id field is present" do
      it "returns nil" do
        expect(subject.send(:apply_work, MockVisibilityWork.new(1001))).to be_nil
      end
    end
  ...
  describe "#build_collection" do  # 395
    ...
    context "when continue_new_service evaluates to true" do
      ...
      context "when find_existing_collection does NOT return existing collection" do
        ...
        before {
          ...
          allow(subject).to receive(:default_resource_type).with("resource type").and_return ["resource type"]
        ...
        context "when mode is MODE_MIGRATE and collection_hash parameter" do
          before {
            allow(subject).to receive(:mode).and_return "migrate"
            allow(subject).to receive(:new_collection).with( creator: ["creator"],
          ...
          it "creates and returns a new collection with the id parameter passed in" do
            expect(subject).to receive(:new_collection).with( creator: ["creator"],
                                                              curation_notes_admin: ["admin"],
                                                              curation_notes_user: ["user"],
                                                              date_created: "date created",
                                                              date_modified: "date modified",
    ...
    context "when id parameter is present" do
      ...
      context "when mode is MODE_MIGRATE" do
        ...
        context "when collection is found" do
          ...
          it "returns collection" do
            expect(subject).to receive(:find_collection_using_id)
  ...
  describe "#default_resource_type" do
    ...
    collections.each do |collection|
      context "when resource_type is equal to #{collection[:resource_type]}" do
        it "returns #{collection[:expected_result]}" do
          expect(subject.send(:default_resource_type, collection[:resource_type])).to eq collection[:expected_result]
        end
      end
    end
  end
          ...
          context "when collection_type_gid is present in hash parameter" do
            context "when result is found for hash parameter collection_type_gid" do
              ...
              it "calls CollectionType.find_by_gid and returns result" do
                expect(Hyrax::CollectionType).to receive(:find_by).with(machine_id: "collection type")
                expect(Hyrax::CollectionType).to receive(:find_by_gid).with("type gid")
                expect(Hyrax::CollectionType).not_to receive(:find_or_create_default_collection_type)

                expect(subject.send(:build_collection_type, hash: {:collection_type => "collection type", :collection_type_gid => "type gid"})).to eq "found type gid"
              end
            end
  ...
  describe "#build_collections" do
    context "when collections evaluates to false" do
      before {
    ...
    context "when collections evaluates to true" do
      before {
      ...
      context "when continue_new_content_service evaluates to true" do
        before {
          measurement = MockCollectionMeasure.new
          allow(subject).to receive(:continue_new_content_service).and_return true
  ...
  describe "#build_date" do
    context "when key parameter value of hash parameter is blank" do
      ...
      it "return result of build_date_now" do
  ...
  describe "#build_date2" do  # TODO:  change to for each loop?
    context "when string matches format /\d\d?\/\d\d?\/\d\d\d\d/" do
      before {
        allow(DateTime).to receive(:strptime).with( "10/31/2025", "%m/%d/%Y" ).and_return "Halloween"
      }
      it "returns string parsed to DateTime" do
        expect(DateTime).to receive(:strptime).with( "10/31/2025", "%m/%d/%Y" )
        expect(subject).not_to receive(:build_date_now)

        expect(subject.send(:build_date2, "10/31/2025", key: "build date 2 key", no_default: false)).to eq "Halloween"
      end
    end

    context "when string matches format /\d\d?\-\d\d?\-\d\d\d\d/" do
      before {
        allow(DateTime).to receive(:strptime).with( "4-20-2025", "%m-%d-%Y" ).and_return "Easter"
      }
      it "returns string parsed to DateTime" do
        expect(DateTime).to receive(:strptime).with( "4-20-2025", "%m-%d-%Y" )
        expect(subject).not_to receive(:build_date_now)

        expect(subject.send(:build_date2, "4-20-2025", key: "build date 2 key", no_default: false)).to eq "Easter"
      end
    end

    context "when string matches format /d\d\d\d/" do
      before {
        allow(DateTime).to receive(:strptime).with( "2025", "%Y" ).and_return "twenty twenty five"
      }
      it "returns string parsed to DateTime" do
        expect(DateTime).to receive(:strptime).with( "2025", "%Y" )
        expect(subject).not_to receive(:build_date_now)

        expect(subject.send(:build_date2, "2025", key: "build date 2 key", no_default: false)).to eq "twenty twenty five"
      end
    end

    context "when string does not match any of the previous formats" do
      before {
        allow(subject).to receive(:build_date_now).with( no_default: false).and_return "build date now"
      }
      it "returns result of build_date_now" do
        expect(subject).to receive(:build_date_now).with( no_default: false)
        expect(DateTime).not_to receive(:strptime)

        expect(subject.send(:build_date2, "not a date", key: "build date 2 key", no_default: false)).to eq "build date now"
      end
    end
  ...
  describe "#build_date_coverage" do
    ...
    context "when date_coverage value of hash parameter is present" do
      it "returns first item of array" do
  ...
  describe "#build_depositor" do
    ...
    context "when depositor value is NOT present in the hash parameter" do
      ...
      it "return result of calling user_key" do
        expect(subject.send(:build_depositor, hash: {:updater => "the user"})).to eq "key"
      end
    end
  end
  ...
  describe "#build_doi" do
    ...
    context "when doi value is NOT present in hash parameter" do
      it "returns nil" do
        expect(subject.send(:build_doi, hash: {:boo => "the boo value"})).to be_nil
      end
    end
  end

  # TODO add log_verbose_msg where sensible
  ...
  describe "#build_file_set" do
    ...
    context "when filename and file_ids parameters are present" do
      before {
        ...
        allow(subject).to receive(:build_file_set_new).with(id: "ID", depositor: "work depositor", path: "path", original_name: "filename", build_mode: "append",
                                                            current_user: "user key").and_return file_set
        ...
        allow(subject).to receive(:build_file_set_ingest).with(file_set: file_set, path: "path", checksum_algorithm: nil, checksum_value: nil, build_mode: "append").and_return file_set
      }
      it "builds and returns file set using filename and file_ids parameters" do
        expect(subject.send(:build_file_set, id: "ID", path: "path", work: OpenStruct.new(depositor: "work depositor", visibility: "visibility"), filename: "filename",
                            file_ids: "file ids", file_set_of: "set", file_set_count: "1", file_size: " 23MB")).present? == true
      end
    end
    ...
    context "when filename and file_ids parameters are blank" do
      before {
        ...
        allow(subject).to receive(:build_file_set_new).with(id: "ID", depositor: "work depositor", path: "path", original_name: "basename path", build_mode: "append",
                                                            current_user: "user key").and_return file_set
      ...
      it "builds and returns file set without using filename and file_ids parameters" do
        expect(subject.send(:build_file_set, id: "ID", path: "path", work: OpenStruct.new(depositor: "work depositor", visibility: "visibility"), filename: nil, file_ids: nil,
                            file_set_of: "set", file_set_count: "1", file_size: " 23MB")).present? == true
        end
    end
  end
  ...
  describe "#build_file_set_from_hash" do
    file_set_hash = {:file_path => "file path", :original_name => "original name", :curation_notes_admin => "admin", :curation_notes_user => "user", :checksum_algorithm => "algorithm",
                     :checksum_value => "checksum value", :edit_users => "edit users", :label => "label", :title => "title"}
  ...
  #       }
  #       it "" do
  #         expect(subject.send(:build_file_set_ingest, file_set: "file set", path: "path", checksum_algorithm: "algorithm", checksum_value: "checksum value", build_mode: "build")).to eq "file set"
  #       end
  #     end
  #
  #     context "when file_set_checksum returns a value NOT equal to the checksum_value parameter" do
  #       before {
  #         allow(subject).to receive(:file_set_checksum).with(file_set: "file set").and_return OpenStruct.new(algorithm: "algorithm", value: "totally different value")
  #         allow(subject).to receive(:log_msg).with "build: WARNING checksum failed: totally different value vs checksum value"
  #         allow(subject).to receive(:log_provenance_fixity_check).with(curation_concern: "file set", fixity_check_status: "failed", fixity_check_note: "totally different value vs checksum value")
  #       }
  #       it "" do
  #         expect(subject.send(:build_file_set_ingest, file_set: "file set", path: "path", checksum_algorithm: "algorithm", checksum_value: "checksum value", build_mode: "build")).to eq "file set"
  #       end
  ...
  #         allow(subject).to receive(:log_provenance_fixity_check).with(curation_concern: "file set", fixity_check_status: "failed", fixity_check_note: "different algorithm altogether vs algorithm")
  #       }
  #       it "" do
  ...
  #       before {
  #         allow(subject).to receive(:file_set_checksum).with(file_set: "file set").and_return ""
  #         allow(subject).to receive(:log_msg).with "build: file checksum is nil"
  #       }
  #       it "" do
  #         subject.send(:build_file_set_ingest, file_set: "file set", path: "path", checksum_algorithm: "algorithm", checksum_value: "checksum value", build_mode: "build")
  #       end
  #     end
  #   end
  #
  ...
  #       expect(subject).to receive(:log_provenance_migrate).with(curation_concern: "file set", build_mode: "build")
  #       expect(Deepblue::IngestHelper).to receive(:characterize).with( "file set",
  #                                                                      nil,
  #                                                                      "path",
  #                                                                      delete_input_file: false,
  #                                                                      continue_job_chain: false,
  ...
  #                                                                      ingester: "ingester",
  #                                                                      ingest_timestamp: "ingest timestamp" )
  #       expect(Deepblue::IngestHelper).to receive(:create_derivatives).with( "file set",
  #                                                                            nil,
  #                                                                            "path",
  #                                                                            delete_input_file: false,
  #                                                                            current_user: "user",
  ...
  #                                                                            ingester: "ingester",
  #                                                                            ingest_timestamp: "ingest timestamp" )
  #       expect(subject).to receive(:log_provenance_ingest).with(curation_concern: "file set")
  ...
  #   #   expect(subject).to have_received(:log_msg).with("build: finished: path")
  #   # }
  # end
  pending "#build_file_set_ingest"
  ...
  describe "#build_file_set_new" do
    ...
    context "when build_mode parameter is MODE_MIGRATE" do
      before {
        ...
        allow(subject).to receive(:new_file_set).with(id: 144).and_return file_set
      }
      it "opens file and uploads file to file set with existing id" do
        ...
        expect(subject).to receive(:new_file_set).with(id: 144).and_return file_set
        expect(file_set).to receive(:apply_depositor_metadata).with "depositor"

        expect(subject.send(:build_file_set_new, id: 144, depositor: "depositor", path: "file path", original_name: "original name", build_mode: "migrate", current_user: "user")).to eq file_set
      end
    end
    ...
    context "when build_mode parameter is NOT MODE_MIGRATE" do
      before {
        ...
        allow(subject).to receive(:new_file_set).with(id: nil).and_return file_set
      }
      it "opens file and uploads file to file set" do
        expect(subject).to receive(:log_msg).with "append: processing: file path"
        expect(file).to receive(:define_singleton_method).with(:original_name)
        expect(file).to receive(:define_singleton_method).with(:current_user)
        expect(subject).to receive(:new_file_set).with(id: nil)
        expect(file_set).to receive(:apply_depositor_metadata).with "depositor"

        expect(subject.send(:build_file_set_new, id: 123, depositor: "depositor", path: "file path", original_name: "original name", build_mode: "append", current_user: "user")).to eq file_set
      end
    end
    ...
    context "when continue_new_content_service evaluates to true" do
      ...
      context "when collection_hash parameter is present, id value is NOT blank, and mode value is MODE_MIGRATE" do
        ...
        context "when build_collection returns collection" do
          ...
          it "returns collection" do
            expect(Collection).not_to receive(:find)
            expect(subject).to receive(:build_collection).with(id: "88", collection_hash: collection_hash)
            expect(subject).to receive(:log_object).with collection
            expect(subject).to receive(:add_works_to_collection).with(collection_hash: collection_hash, collection: collection)
            expect(collection).to receive(:save!)
            expect(subject.send(:build_or_find_collection, collection_hash: collection_hash)).to eq collection
  ...
  describe "#build_or_find_user" do  # 727

  end

  pending "intervening tests"


  describe "#diff_file_set" do  # 1076
    context "when continue_new_content_service evaluates to false" do
      before {
    ...
    context "when continue_new_content_service evaluates to true" do
      ...
      before {
        ...
        #  NOTE: 'orignal_name' is misspelled
        allow(subject).to receive(:diff_value_value).with( "diffs", file_set, attr_name: :orignal_name, current_value: file_set.original_name_value, value: "the original" )
      ...
      it "calls functions with and returns diffs" do
        ...
        expect(subject).to receive(:diff_value_value).with( "diffs", file_set, attr_name: :orignal_name, current_value: file_set.original_name_value, value: "the original" )
  ...
  describe "#diff_file_sets" do
    ...
    context "when file_set_ids are present in work_hash parameter" do
      before {
        allow(subject).to receive(:diff_file_sets_from_file_set_ids).with(diffs: "diffs", work_hash: {:file_set_ids => "file set ids"}, work: "work").and_return "diff file sets"
      }
  ...
  describe "#diff_file_sets_from_file_set_ids" do  # 1105
    context "when continue_new_content_service evaluates to false" do
      before {
        allow(subject).to receive(:continue_new_content_service).and_return nil
      }
  ...
  #     file_set_2 = OpenStruct.new(id: 2)
  #     file_set_3 = OpenStruct.new(id: 3)
  #     work = OpenStruct.new(file_sets: [file_set_1, file_set_2, file_set_3])
  #
  #     before {
  #       allow(subject).to receive(:continue_new_content_service).and_return true
  #       allow(subject).to receive(:diff_file_set).with(diffs: "differences", file_set: file_set_1, file_set_hash: file_set_hash, parent: work )
  #     }
  #     it "returns diffs" do
  #       work_hash = {:file_set_ids => [1, 2, 3]}
  #
  #       expect(subject.send(:diff_file_sets_from_file_set_ids, diffs: "differences", work_hash: work_hash, work:work )).to eq "differences"
  #     end
  #
  #     context "when " do
  #       it "" do
  ...
  describe "#diff_users" do
    ...
    context "when users function returns value(s)" do
      before {
        ...
        allow(subject).to receive(:users).and_return [ {:user_emails => ["charlotte"]} ]
      }
  ...
  describe "#diff_user_attr" do
    ...
    context "when calling diff_user_attr? on attr_name parameter as a symbol returns true" do
      ...
      context "when attr_name_hash parameter is blank" do
        before {
          allow(subject).to receive(:diff_user_attr_if_blank?).with(:attr_name, value: "valuation").and_return true
        }
      ...
      context "when attr_name_hash parameter is present" do
        before {
          allow(subject).to receive(:diff_user_attr_if_blank?).with(:attr_name, value: "hashed").and_return true
        }
        it "appends parameters to and returns diffs parameter" do
      ...
      context "when diff_user_attr_if_blank? returns false" do
        before {
          allow(subject).to receive(:diff_user_attr_if_blank?).with(:attr_name, value: "valuation").and_return false
        }
        it "returns diffs parameter" do
          expect(subject.send(:diff_user_attr, "diffs ", {:attr_name => "attr current"}, {:attr_name => "valuation"}, "user email",
      ...
      context "when attr_current == value" do   #ToDo: update language
        before {
          allow(subject).to receive(:diff_user_attr_if_blank?).with(:attr_name, value: "attr current").and_return true
        }
        it "returns diffs parameter" do
      ...
      context "when Exception occurs" do
        before {
          allow(subject).to receive(:diff_user_attr_if_blank?).with(:attr_name, value: "valuation").and_raise(Exception, "Error Message")
        }
  ...
  describe "#diff_user_attr_if_blank?" do  # parent parameter not in use
    context "when value parameter is blank" do
      it "returns false" do
        expect(subject.send(:diff_user_attr_if_blank?, "attr name", value: "")).to eq false
      end
    end
    context "when value parameter is present" do
      it "returns true" do
        expect(subject.send(:diff_user_attr_if_blank?, "attr name", value: "not blank")).to eq true
      end
    end
  end
  ...
  describe "#diff_value_value" do
    ...
    context "when diff_attr_if_blank? evaluates to false" do
      before {
        ...
        allow(subject).to receive(:diff_attr_if_blank?).with("attr name", value: nil).and_return nil
      }
      it "returns diffs parameter" do
        expect(subject.send(:diff_value_value, "diffs", "cc_or_fs", attr_name: "attr name", current_value: nil)).to eq "diffs"
      end
    ...
    context "when current_value parameter equals value parameter" do
      before {
        allow(subject).to receive(:diff_attr?).with("attr name").and_return "success"
        allow(subject).to receive(:diff_attr_if_blank?).with("attr name", value: "valuation").and_return "success"
      }
      it "returns diffs parameter" do
        expect(subject.send(:diff_value_value, "diffs", "cc_or_fs", attr_name: "attr name", current_value: "valuation", value: "valuation")).to eq "diffs"
      end
    end
  ...
  describe "#diff works" do
    skip "add tests"
  end


  describe "#diff_works" do  # 1260
    ...
    context "when works returns results" do
      ...
      it "" do
        # expect(subject).to receive(:puts).with "== work work id 2 is missing =="
        # expect(subject).to receive(:puts).with "attr prefix work 3: diff..."
        # expect(subject).to receive(:puts).with "attr prefix work 3: diffs"
        # expect(subject).to receive(:puts).with "diff1\ndiff2"
        #
        # expect(subject).to receive(:puts).with "attr prefix work 4: diff..."
        # expect(subject).to receive(:puts).with "attr prefix work 4: no differences found"
        #
        # expect(subject).to receive(:add_measurement).with("measurement").twice
        #
  ...
  describe "#doi_mint" do
    ...
    context "when calling doi_mint on curation_concern parameter raises an Exception" do  # removed 'work' variable
      before {
  ...
  describe "#find_collection" do
    ...
    context "when parameter is present and continue_new_content_service evaluates to true" do
      before {
        allow(subject).to receive(:continue_new_content_service).and_return true
  ...
  describe "#find_collection_using_id" do
    ...
    context "when id parameter is present" do
      ...
      context "when Collection with id canNOT be found" do
        before {
          allow(Collection).to receive(:find).with("collection id").and_raise(ActiveFedora::ObjectNotFoundError)  # TODO: use this other place too
        }
  ...
  describe "#find_collection_using_prior_id" do
    ...
    context "when prior_id parameter is present" do
      ...
      cc2 = OpenStruct.new(prior_identifier: "prior ID")
  ...
  describe "#find_collections_and_update" do
    before {
      subject.instance_variable_set(:@label, "label")
    }
    ...
    context "when collections is present" do
      ...
      before {
        ...
        allow(Benchmark).to receive(:measure).and_return measurement
        allow(subject).to receive(:find_collection).with(collection_hash: "collection hash").and_return ["collection", "collection ID"]
        allow(subject).to receive(:update_collection_from_hash).with(collection_hash: "collection hash", collection: "")

        allow(subject).to receive(:find_collection).with(collection_hash: "collection hash 2").and_return ["collection", "collection ID"]
        allow(subject).to receive(:update_collection_from_hash).with(collection_hash: "collection hash 2", collection: "collection")

        allow(subject).to receive(:add_measurement).with measurement
      }
      context "when object in collections is present" do
        it "sets instance variable and adds measurement" do
          #expect(measurement).to receive(:instance_variable_set).with("label", "collection ID")
          expect(subject).to receive(:collections)
          expect(subject).to receive(:user_create_users).with(emails: "user key")
          expect(subject).to receive(:continue_new_content_service)
          expect(Benchmark).to receive(:measure)
      ...
      skip "add a test for @label instance variable"
      skip "add a test for add_measurement"
  ...
  describe "#find_file_set_using_prior_id" do
    context "when prior_id parameter is blank" do
      it "returns nil" do
        expect(subject.send(:find_file_set_using_prior_id, prior_id: nil, parent: "parent")).to be_nil
      end
    end

    context "when parent parameter is blank" do
      it "returns nil" do
        expect(subject.send(:find_file_set_using_prior_id, prior_id: "prior ID", parent: nil)).to be_nil
      end
    end
  ...
  describe "#find_or_create_user" do
    ...
    context "when user can be found" do
      before {
        allow(User).to receive(:find_by).with(user_key: "user key").and_return "found user"
      }
      it "returns user" do
        expect(User).to receive(:find_by).with(user_key: "user key")

        expect(subject.send(:find_or_create_user)).to eq "found user"
      end
    end
    ...
    context "when user canNOT be found" do
      context "when user can be created" do
        it "creates and returns user" do
          skip "add a test"                  # NOTE: does create_user function exist?
        end
      end
  ...
  describe "#find_work" do
    ...
    context "when ActiveFedora::ObjectNotFoundError is raised" do
      skip "add a test"
    end
  end

  describe "#find_works_and_add_files" do  #1428
    ...
    context "when works is present" do
      ...
      before {
        ...
        # allow(subject).to receive(:build_depositor).with(hash: "work hash")
      ...
      it "calls Benchmark.measure and adds measurement" do
        expect(subject).to receive(:works)
        expect(subject).to receive(:continue_new_content_service)
        expect(subject).to receive(:find_work).with(work_hash: "work hash")

        expect(Benchmark).to receive(:measure).with("work id")
        # do
        #   expect(subject).to receive(:add_file_sets_to_work).with( work_hash: "work hash", work: work )
        #   expect(subject).to receive(:build_depositor).with(hash: "work hash")
        #   expect(subject).to receive(:build_admin_set_work).with(hash: "work hash")
        #   expect(subject).to receive(:apply_visibility_and_workflow).with(work: work, work_hash: "work hash", admin_set: "admin set")
        #   expect(subject).to receive(:log_object).with work
        # end
        expect(subject).to receive(:add_measurement).with "measurement"
  ...
  describe "#find_work_using_prior_id" do
    ...
    context "when prior_id parameter is present" do
      ...
      context "when parent parameter is blank" do
        ...
        context "when prior id exists in all works" do
        ...
        context "when prior id does NOT exist in all works" do
          before {
  ...
  describe "#ingester" do
    # before {
    #   user_ingester = ActiveSupport::HashWithIndifferentAccess.new(user: {:ingester => "user ingester"})
    #   subject.instance_variable_set :@ingester, user_ingester
    # }
    # it "returns value from @cfg_hash" do
    #   expect(subject.send(:ingester)).to eq "user ingester"
    # end
    skip "add a test"
  end
  ...
  describe "#initialize_with_msg" do
    ...
    subject { Deepblue::NewContentService.new(path_to_yaml_file: "yaml path", cfg_hash: "cfg hash", base_path: "base path", options: options) }  #calls the method
    ...
    before {
      ...
      # File, Pathname, and Process cannot be stubbed
    }

    context "when options keys does not include error and verbose is false" do
      it "sets instance variables" do
        expect(Deepblue::TaskHelper).to receive(:task_options_parse)
        expect(Deepblue::TaskHelper).to receive(:task_options_value)
    ...
    skip "add a test for DeepBlueDocs::Application.config.provenance_log_echo_to_rails_logger = false"
    skip "add a test for error key in options"
    skip "add a test for verbose = true"
    skip "add a test for merging config values"
    skip "add a test with mode parameter"
    skip "add a test with ingester parameter"
    skip "add a test without user_create parameter"
    skip "add a test without msg parameter"
    skip "add a test for @stop_new_content_service_file and @stop_new_content_service_ppid_file"
  end


  describe "#log_msg" do   #1532
  ...
  describe "#log_provenance_add_child" do
    ...
    context "when parent parameter responds to provenance_child_add" do
      before {
        allow(subject).to receive(:user).and_return "user"  # TODO:  put at top of file
        allow(subject).to receive(:ingest_id).and_return "ingest id"
        allow(subject).to receive(:ingester).and_return "ingester"
        allow(subject).to receive(:ingest_timestamp).and_return "ingest timestamp"
      }
  ...
  describe "#measurements" do
    ...
    context "when @measurements has NO value" do
      it "sets @measurements to empty array" do
  ...
  pending "#new_collection"  # TODO:  needed, or delete function in NewContentService

  describe "#new_data_set" do  #1676
    context "id parameter is present" do  # only called once in NewContentService
      before {
        allow(DataSet).to receive(:new).with(authoremail: "author email",
                                             ...
                                             doi: "doi",
                                             fundedby: "funded by",
                                             fundedby_other: "funded by other",
      ...
      # it "calls DataSet.new with id parameter" do
      ...
      #                                         title: "The Title")
      #
      #   subject.send(:new_data_set, authoremail: "author email",
      #                               contributor: 'contributor',
      #                               creator: "creator",
      #                               curation_notes_admin: "admin",
      #                               curation_notes_user: "user",
    ...
    context "when id parameter is blank" do
      # never called - removing function
    end
  end


  describe "#new_file_set" do  # TODO: remove next
    context "id parameter is present" do
      before {
        allow(FileSet).to receive(:new).with(id: "it's me")
      }

      it "calls FileSet.new with id parameter" do
        expect(FileSet).to receive(:new).with(id: "it's me")

        subject.send(:new_file_set, id: "it's me")
      end
    end

    context "when id parameter is blank" do
      before {
        allow(FileSet).to receive(:new).with(no_args)
      }

      it "calls FileSet.new WITHOUT id parameter" do
        expect(FileSet).to receive(:new).with(no_args)

        subject.send(:new_file_set, id: nil)
      end
    end
  end
  ...
  describe "report" do
    ...
    context "when measurements parameter is present" do
      measuring = [MockMeasurement.new("label1", 123), MockMeasurement.new("label2", 456), MockMeasurement.new("label3", 789)]
  ...
  describe "#source" do
    ...
    context "when @source is nil" do
      ...
      it "calls valid_restricted_vocab to set the value of @source" do
  ...
  describe "#update_attr_if_blank?" do   # NOTE: attr_name and parent parameters not in use
    context "when value parameter is blank" do
      it "returns false" do
        expect(subject.send(:update_attr_if_blank?, "attr name", value: "")).to eq false
      end
    end

    context "when value parameter is present" do
      it "returns true" do
        expect(subject.send(:update_attr_if_blank?, "attr name", value: "almond")).to eq true
      end
    end
  end
  ...
  describe "#update_collection_from_hash" do  # NOTE: collection_hash and collection parameters not in use
    it "returns nil" do
      expect(subject.send(:update_collection_from_hash, collection_hash: "co hash", collection: "collection")).to be_nil
    end
  end
  ...
  describe "#update_file_sets_from_files" do  # NOTE: work_hash and work parameters are not in use
    it "returns updates parameter" do
      expect(subject.send(:update_file_sets_from_files, updates: "hot off the presses", work_hash: nil, work: nil)).to eq "hot off the presses"
    end
  end
  ...
  describe "#update_user_attr_if_blank?" do  # NOTE: attr_name and parent parameters are not in use
    context "when value parameter is blank" do
      it "returns false" do
        expect(subject.send(:update_user_attr_if_blank?, "attr name", value: "")).to eq false
      end
    end

    context "when value parameter is present" do
      it "returns true" do
        expect(subject.send(:update_user_attr_if_blank?, "attr name", value: "pistachio")).to eq true
      end
    end
  end
  ...
  describe "#upload_file_to_file_set" do
    context "when no Exception occurs" do
      ...
      it "returns true" do
        expect(Hydra::Works::UploadFileToFileSet).to receive(:call).with( "file set", "a file" )
    ...
    context "when user_create evaluates to positive" do
      ...
      context "when emails parameter has a value" do
        ...
        it "creates and logs new user(s)" do
          ...
          #expect(created_user).to receive(:save).with(validate: false)
        ...
        skip "add test for created_user results"
      end
    end
  ...
  describe "#users_from_hash" do
    it "returns value of :users key from hash parameter in an array" do
  ...
  describe "#validate_config" do #2273
    context "when " do
      it " " do
        skip "Add tests"
      end
  ...
  describe "#valid_restricted_vocab" do
    ...
    context "when vocab does not include value" do
      it "raises RestrictedVocabularyError" do
        expect(subject.send(:valid_restricted_vocab, "off-white", var: "color", vocab: ["eggshell", "ecru"])).to raise_error(Deepblue::NewContentService::RestrictedVocabularyError,
  ...
  describe "#visibility" do
    context "when @visibility has a value" do
      ...
      it "return @visibility" do
        expect(subject).not_to receive(:visibility_curation_concern).with("privacy")
  ...
  describe "#visibility_curation_concern" do
    before {
      allow(subject).to receive(:valid_restricted_vocab).with("vis", var: :visibility, vocab: %w[open restricted], error_class: Deepblue::NewContentService::VisibilityError)
    }
    it "calls valid_restricted_vocab" do
      expect(subject).to receive(:valid_restricted_vocab).with("vis", var: :visibility, vocab: %w[open restricted], error_class: Deepblue::NewContentService::VisibilityError)
      subject.send(:visibility_curation_concern, "vis")
    end
  end
