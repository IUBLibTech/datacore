...
RSpec.describe Deepblue::NewContentService do
      ...
      context "when user is NOT found by email" do
        ...
        context "when build_user returns user" do
          ...
          it "builds and returns user" do
            expect(subject).to receive(:log_verbose_msg).with("build_or_find_user: email: exampleatimaginarydotorg", verbose: false)
            expect(User).to receive(:find_by_user_key).with("exampleatimaginarydotorg")
            expect(subject).to receive(:build_user).with(user_hash: user_hash)
      ...
      context "when work_hash parameter is present" do
        ...
        context "when id is present and mode is MODE_APPEND" do
          ...
          context "when work is found" do
            ...
            it "returns work" do
              expect(subject).to receive(:comment_work).with(work_hash: work_hash)
              expect(subject).to receive(:find_work_using_id).with(id: "78")
  ...
  describe "#build_repo_contents" do
    it "returns nil" do
      expect(subject.send(:build_repo_contents)).to be_nil
    end
  end
  ...
  describe "#build_user" do
    ...
    it "creates new user and logs message" do
      ...
      #expect(new_user).to receive(:save).with(validate: false)
  ...
  describe "find_existing_work" do
    ...
    modes.each do |mode|
      context "when mode is #{mode} and id parameter is blank" do
        before {
          allow(subject).to receive(:mode).and_return mode
        }
        it "returns nil" do
          expect(subject.send(:find_existing_work, id: "")).to be_nil
        end
      end
    end
    ...
    context "when id parameter is present" do
      ...
      context "when mode is NOT MODE_APPEND or MODE_MIGRATE" do   # for example, MODE_UPDATE
        ...
        it "returns nil" do
          expect(subject.send(:find_existing_work, id: 606)).to be_nil
        end
      end
    end
  end
  ...
  describe "#value_from_attr" do
    ...
    hash_it_out.each do |hash|
      context "when attr_name_hash parameter is #{hash[:desc]} and multi parameter is #{hash[:multi]}" do
        it "returns cc_or_fs_hash parameter #{hash[:return_value]} value #{hash[:multi] ? 'in an array' : ''}" do
  ...
  describe "#diff_attr?" do
    context "when diff_attrs_skip does not include the attribute name" do
      ...
      it "returns true" do
        expect(subject.send(:diff_attr?, "attr_name"))
      end
    end
  ...
  describe "#diff_collection_works" do
    context "when continue_content_service is false" do
      ...
      context "when collection parameter has member_objects" do
        it "returns diffs parameter" do
          expect(subject.send(:diff_collection_works, diffs: "diffs", collection: OpenStruct.new(member_objects: ["objects"]), collection_hash: nil)).to eq "diffs"
        end
      end
  ...
  describe "#diff_user_attr" do
    ...
    context "when calling diff_user_attr? on attr_name parameter as a symbol returns true" do
      ...
      after {
        expect(subject).to have_received(:diff_user_attr_if_blank?)
      }
    end
    ...
    context "when works returns results" do
      ...
      context "when continue_new_content_service returns true" do
        ...
        it "calls Benchmark.measure" do
          ...
          expect(subject).to receive(:continue_new_content_service)
          expect(Benchmark).to receive(:measure)
          #expect(measurement).to receive(:instance_variable_set).with(:@label, "work id 3")
  ...
  describe "#find_collections_and_update" do
    ...
    context "when collections is present" do
      ...
      #     expect(Benchmark).not to receive(:measure)
      #
    ...
    # after {
    #   expect(subject).to have_received(:collections)
    # }
  end
  ...
  #         }
  #         it "returns the updates parameter" do
  #           #expect(subject).to have_received(:update_attr_if_blank?).with( value: "attr_current")
  #           expect(subject.send(:update_attr, "updates", {"attr_name" => "attr_current"}, "cc_or_fs_hash", attr_name: "attr_name")).to eq "updates"
  #         end
  #       end
  #
  ...
  #       before {
  #         allow(subject).to receive(:value_from_attr).with("cc_or_fs_hash", attr_name: "attr_name", attr_name_hash: nil, multi: true).and_return "a value"
  #         allow(subject).to receive(:update_attr_if_blank?).with( value: "a value").and_raise(Exception, "an error")
  #         allow(subject).to receive(:attr_prefix).with({"attr_name" => "a value"}).and_return " prefix"
  #       }
  #
  ...
  describe "#update_attr_doi" do
    context "when cc_or_fs_hash[:doi] parameter value is NOT equal to 'mint_now'" do
      before {
        allow(subject).to receive(:update_attr).with("updates", "cc_or_fs", {:doi => "mint_then"}, attr_name: :doi, multi: false).and_return "update attr"
      }
      it "returns reuslt of update_attr function" do
        expect(subject.send(:update_attr_doi, "updates", "cc_or_fs", {:doi => "mint_then"})).to eq "update attr"
      end
    end

    context "when allow_minting parameter is false" do
      before {
        allow(subject).to receive(:update_attr).with("updates", "cc_or_fs", {:doi => "mint_now"}, attr_name: :doi, multi: false).and_return "update attr"
      }
      it "returns result of update_attr function" do
        expect(subject.send(:update_attr_doi, "updates", "cc_or_fs", {:doi => "mint_now"}, allow_minting: false)).to eq "update attr"
      end
    end
    ...
    context "when collections function returns results" do
      ...
      context "when continue_new_content_service function returns true" do
        ...
        it "finds and updates collections and returns result of collections function" do
          ...
          expect(subject).to receive(:continue_new_content_service).twice
          expect(Benchmark).to receive(:measure)
          #expect(measurement).to receive(:instance_variable_set).with(:@label, "collection id 1")
          expect(subject.send(:update_collections)).to eq [nil, "collection hash 1", "collection hash 2"]
        end
  ...
  describe "#update_edit_users" do
    ...
    context "when calling diff_attr? with :edit_users returns true" do
      ...
      context "when diff_attr_if_blank? returns true" do
        before {
          allow(subject).to receive(:diff_attr_if_blank?).with(:edit_users, value: ["them"]).and_return true
        }
        it " " do
          #expect(subject).to receive(:diff_attr_if_blank?)
          #expect(subject.send(:update_edit_users, "updates", OpenStruct.new(edit_users: "edit users"), {:edit_users => "them"})).to eq "updates"
  ...
  describe "#update_file_set" do
    ...
    context "when continue_new_content_service returns true" do
      ...
      context "when updates parameter is NOT empty" do
        ...
        skip "add test for saving file set"
      end
  ...
  describe "#update_user" do
    context "when user_email parameter is nil" do
      ...
      #   subject.send(:update_user, user: user, user_hash:{:age => "21"})
      # end
  ...
  describe "#update_users" do
    ...
    context "when the users function returns results" do
      ...
      it "calls Benchmark.measure, updates users, and returns result" do
        expect(Benchmark).to receive(:measure)
        # expect(user_r).to receive(:save).with(validate: false)
    ...
    skip "add a test for functions inside of Benchmark.measure"
  end
  ...
  describe "#update_work" do
    context "when continue_new_content_service returns false" do
      ...
      it 'returns updates parameter' do
        expect(subject.send(:update_work, updates: "updates", work_hash: "work hash", work: "work"))
      end
    end
    ...
    context "when continue_new_content_service returns true" do
      ...
      before {
        ...
        allow(subject).to receive(:update_file_sets).with( updates: [], work_hash: work_hash, work: work )
      ...
      context "when the updates parameter is NOT empty" do
      ...
      context "when the updates parameter is empty" do
        it 'saves the work parameter and returns update_attr_doi updates' do
          expect(work).to receive(:save!)
          expect(subject.send(:update_work, updates: [], work_hash: work_hash, work: work)).to eq ["updated"]
        end
      end
    end
  end
